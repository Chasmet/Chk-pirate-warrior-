...
const WORLD_MIN_X_V7 := -650.0
const WORLD_MAX_X_V7 := 5700.0
const WORLD_MIN_Z_V7 := -1500.0
const WORLD_MAX_Z_V7 := 1600.0
...
func configure(data: Dictionary) -> void:
	...
	print("CHK_WORLD_V7_ARCHIPELAGO_READY zones=%d island_scale=%.2f" % [ZONES_V4.size(), ISLAND_SCALE_V5])
...
func _keep_player_in_world() -> void:
	if not is_instance_valid(player):
		return
	if player.boat_mode:
		var bounded_x := clampf(player.global_position.x, WORLD_MIN_X_V7, WORLD_MAX_X_V7)
		var bounded_z := clampf(player.global_position.z, WORLD_MIN_Z_V7, WORLD_MAX_Z_V7)
		...
		return
	var zone := zones_v5[current_zone] as Dictionary
...
func get_dock_position(zone_index: int, water_side: bool) -> Vector3:
	var zone := zones_v5[clampi(zone_index, 0, zones_v5.size() - 1)] as Dictionary
	var center := Vector3(zone["center"])
	var direction := Vector3(zone["dock_dir"]).normalized()
...
func _spawn_wave() -> void:
	...
	for i in range(10):
		...
		enemy.global_position = center + Vector3(cos(angle) * radius, float(zone.get("elevation", 0.0)) + 8.0, sin(angle) * radius)
...
func _spawn_boss() -> void:
	...
	var zone := zones_v5[current_zone] as Dictionary
	var center := Vector3(zone["center"])
	boss.global_position = center + Vector3(0, float(zone.get("elevation", 0.0)) + 8.0, -float(zone["radius"]) * 0.40)
...
func _restore_exact_snapshot() -> void:
	...
	if not _saved_position_is_valid(saved_position):
		push_warning("Sauvegarde ignorée : position exacte invalide ou issue de l’ancien archipel")
		return
...
func _saved_position_is_valid(value: Vector3) -> bool:
	if not value.is_finite():
		return false
	if value.x < WORLD_MIN_X_V7 or value.x > WORLD_MAX_X_V7 or value.z < WORLD_MIN_Z_V7 or value.z > WORLD_MAX_Z_V7 or value.y < -20.0 or value.y > 280.0:
		return false
...
func _expanded_zones_v5() -> Array:
	...
	for index in range(ZONES_V4.size()):
		var source_zone := ZONES_V4[index] as Dictionary
		var zone: Dictionary = source_zone.duplicate(true)
		var center := _archipelago_center_v7(index)
		zone["center"] = center
		zone["spawn"] = center + Vector3.UP * 8.0
		zone["radius"] = float(zone["radius"]) * ISLAND_SCALE_V5
		result.append(zone)
	return result

func _archipelago_center_v7(index: int) -> Vector3:
	match index:
		0:
			return Vector3(0.0, 0.0, 0.0)
		1:
			return Vector3(720.0, 0.0, -420.0)
		2:
			return Vector3(1580.0, 0.0, -160.0)
		3:
			return Vector3(700.0, 0.0, 720.0)
		4:
			return Vector3(1580.0, 0.0, 820.0)
		5:
			return Vector3(2460.0, 0.0, 260.0)
		6:
			return Vector3(3300.0, 0.0, -560.0)
		7:
			return Vector3(4140.0, 0.0, 540.0)
		8:
			return Vector3(5050.0, 48.0, -220.0)
		_:
			return Vector3.ZERO
