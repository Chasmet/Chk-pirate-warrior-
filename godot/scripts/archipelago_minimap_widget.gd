...
const ZONE_RADII := [13.0, 15.0, 13.0, 15.0, 13.0, 16.0]
...
func _draw() -> void:
	...
	draw_string(font, Vector2(12.0, 22.0), "ARCHIPEL DES QUINET", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 15, Color("f5d985"))
	_draw_compass(font)
	...
	for index in range(ZONE_POINTS.size()):
		...
		draw_string(font, point + Vector2(-22.0, ZONE_RADII[index] + 14.0), ZONE_LABELS[index], HORIZONTAL_ALIGNMENT_CENTER, 44.0, 9, label_color)
