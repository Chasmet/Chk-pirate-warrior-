...
func _run() -> void:
	...
	if sprite != null:
		...
		_check(
			...
			"la texture JPEG Android de dos est importée avec sa transparence mobile"
		)
	...
	if sprite != null:
		_check(sprite.texture.resource_path.ends_with("cheikh_poses.jpg"), "la vue avant affiche réellement le visage de Cheikh dans le monde 3D")
	...
	if pilot_sprite != null:
		_check(pilot_sprite.texture.resource_path.ends_with("nelvyn_steering_v24.jpg"), "la pose de pilotage JPEG est rendue dans le monde 3D")
